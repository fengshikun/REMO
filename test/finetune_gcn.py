    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if args.adv:
            ...
            for t in range(adv.K):
                ...
                #loss matrix after removing null target
                loss_mat = torch.where(is_valid, loss_mat, torch.zeros(loss_mat.shape).to(loss_mat.device).to(loss_mat.dtype))
                loss_adv = torch.sum(loss_mat)/torch.sum(is_valid)
                loss_adv.backward() # 
            adv.restore() #
...
def main():
    ...
    parser.add_argument('--K', type=int, default=3, help='adversarial training  K')
    ...
    root_dataset = '/data/protein/SKData/GraphMAE/dataset'
    #set up dataset
