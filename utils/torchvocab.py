...
# from data_processing import atom_to_vocab
# from data_processing import bond_to_vocab
...
class MolVocab(TorchVocab):
    def __init__(self, file_path, max_size=None, min_freq=1, vocab_type='atom'):
        if vocab_type in ('atom', 'bond'):
            self.vocab_type = vocab_type
        else:
            raise ValueError('Wrong input for vocab_type!')
        ...
        for smi in tqdm.tqdm(smiles):
            ...
            if self.vocab_type == 'atom':
                ...
                for _, bond in enumerate(mol.GetBonds()):
                    ...
                    counter[v] += 1
        print("Building %s vocab from smiles: %d" % (self.vocab_type, len(counter)))
    ...
    def read_smiles_from_file(file_path, start, end, vocab_type):
        ...
        for i, smi in enumerate(smiles):
            ...
            if len(smi.split()) > 1: # contains iupac
                smi = smi.split()[0]
            if not smi: # An empty row of smiles
                print(smi)
                continue
            mol = Chem.MolFromSmiles(smi)
            if vocab_type == 'atom':
