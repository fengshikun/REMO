...
def get_mask_atom_feature(large_dim=False):
    if large_dim:
        atom_feature = [
        ...
        atom_feature = [
            allowable_features['possible_atomic_num_list'].index('misc'),
            0
        ]
    return atom_feature
...
def atom_to_feature_vector(atom, dim_large=False):
    """
    Converts rdkit atom object to feature list of indices
    :param mol: rdkit atom object
    :return: list

    Suffix: the last number is the AtomIdx, which would in line with the reaction centre marker
    """
    if dim_large:
        atom_feature = [
...
def get_bond_mask_feature(large_dim=False):
    if large_dim:
        bond_feature = [
        ...
        bond_feature = [
            safe_index(allowable_features['possible_bond_type_list'], '[SELF]'),
            0
        ]
    return bond_feature
