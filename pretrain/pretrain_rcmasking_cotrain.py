...
class ReactantStage2(torch.nn.Module):
    ...
    def __init__(self, gnn, emb_size, stage1_on=False, stage_one_head_model=None):
        ...
        self.linear_pred_mlabes = torch.nn.Linear(emb_size * 2, 2458)
        if stage1_on:
    ...
    for step, batch in enumerate(train_loader):        
        ...
        if adv is not None:
            ...
            for t in range(adv.K):
                ...
                if org_dataloader_iterator is not None:
                    loss_adv = loss_mlabel + org_loss
                else:
                    loss_adv = loss_mlabel
                loss_adv.backward() # 
            adv.restore() # 
        ...
        if step % 10000 == 1 and not cfg.training_settings.testing_stage:
            if org_dataloader_iterator is not None:
...
def main(cfg):
    ...
    if cfg.training_settings.stage1_on:
        ...
        dataset = MoleculeDataset("{}/".format(root_dataset) + cfg.training_settings.dataset, dataset=cfg.training_settings.dataset, transform = MaskAtom(num_atom_type = 119, num_edge_type = 5, mask_rate = cfg.model.mask_rate, mask_edge=cfg.model.mask_edge))
