...
def train(cfg, model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        graph_rep = molbert(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
...
def eval(cfg, model, device, loader, name="default"):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        with torch.no_grad():
            graph_rep = graphormer(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
            pred = linear_head(graph_rep.view(graph_rep.size(0)//2, -1))
            even_indices = torch.arange(0, graph_rep.size(0)) % 2 == 0
...
def finetune(cfg, benchmark, device):
    ...
    mol_bert_model = GNN_graphpred(cfg.model.num_layer, cfg.model.emb_dim, cfg.model.encoder_embed_dim, JK = cfg.model.JK, drop_ratio = cfg.model.dropout_ratio, graph_pooling = cfg.model.graph_pooling, gnn_type = cfg.model.gnn_type)
    if not cfg.model.input_model_file == "None":
        print('Not from scratch')
        mol_bert_model.from_pretrained(cfg.model.input_model_file)
    mol_bert_model.to(device)
    
    linear_head = RobertaHead(cfg.model.encoder_embed_dim * 2, cfg.model.num_tasks, regression=False).to(device)
    ...
    model_param_group = []
    model_param_group.append({"params": mol_bert_model.gnn.parameters()})
    if cfg.model.graph_pooling == "attention":
        model_param_group.append({"params": mol_bert_model.pool.parameters(), "lr":cfg.training_settings.lr*cfg.training_settings.lr_scale})
    model_param_group.append({"params": mol_bert_model.graph_pred_linear.parameters(), "lr":cfg.training_settings.lr*cfg.training_settings.lr_scale})
    ...
    model = [mol_bert_model, linear_head]
...
@hydra.main(version_base=None, config_path="/home/user/molecular/Chemical_Reaction_Pretraining/conf", config_name="graph_log_ddi")
def main(cfg):
    ...
    wandb.init(project="Chemical-Reaction-DDI-Finetune", name=cfg.wandb.run_name)
